    ...
    with GitRepoFixture():
        ...
        def mock_run_git_command(cmd, repo_path):
            ...
            if cmd[0] == "git" and cmd[1] == "checkout" and cmd[2] == "main":
                print(
                    f"DEBUG: Checkout to main #{len([c for c in git_calls if c[:3] == ['git', 'checkout', 'main']])}"
                )
                ...
                print(
                    f"DEBUG: has_gh_command={has_gh_command}, has_push_command={has_push_command}"
                )
                print(
                    f"DEBUG: Recent commands: {git_calls[-3:] if len(git_calls) >= 3 else git_calls}"
                )
                ...
                if has_gh_command and has_push_command:
                    ...
                    return (
                        False,
                        "Error: Unable to checkout main - working tree has staged changes",
                    )
                else:
        ...
        with patch.object(main, "run_git_command", side_effect=mock_run_git_command):
            ...
            if "Error switching back to main" in result:
                print(
                    "✅ SUCCESS: Final checkout failure properly caught and reported!"
                )
...
def test_final_checkout_success_verification():
    ...
    with GitRepoFixture():
        ...
        # Make some changes
        Path("test_final_checkout_success.txt").write_text(
            "Testing final checkout success"
        )
        time.sleep(2)  # Wait for auto-commit
