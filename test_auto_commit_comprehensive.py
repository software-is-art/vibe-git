...
def test_auto_commit_worker_pre_commit_success():
    ...
    with GitRepoFixture():
        ...
        with patch.object(main, 'run_git_command', side_effect=mock_run_git_command):
            ...
            assert len(commit_commands) == 1, f"Expected 1 commit command, got {len(commit_commands)}"
            assert "--no-verify" not in commit_commands[0], "Should not use --no-verify when pre-commit succeeds"
...
def test_auto_commit_worker_pre_commit_fixes_files():
    ...
    with GitRepoFixture():
        ...
        with patch.object(main, 'run_git_command', side_effect=mock_run_git_command):
            ...
            commit_commands = [cmd for cmd in git_calls if cmd[0] == "commit"]
            assert len(commit_commands) == 2, f"Expected 2 commit attempts, got {len(commit_commands)}"
            
            # Both should be without --no-verify
            for cmd in commit_commands:
                assert "--no-verify" not in cmd, "Should not use --no-verify even when retrying"
    ...
    with GitRepoFixture():
        ...
        def mock_run_git_command(cmd, repo_path):
            ...
            if cmd == ["status", "--porcelain"]:
                ...
                return False, "`pre-commit` not found. Did you forget to activate your virtualenv?"
            elif cmd == ["reset", "HEAD"]:
                return True, ""
            else:
                return True, ""
        ...
        with patch.object(main, 'run_git_command', side_effect=mock_run_git_command):
            ...
            assert len(commit_commands) == 1, f"Expected 1 commit attempt, got {len(commit_commands)}"
            assert len(reset_commands) == 1, f"Expected 1 reset command, got {len(reset_commands)}"
            assert "--no-verify" not in commit_commands[0], "Should not use --no-verify fallback"
...
def test_auto_commit_worker_syntax_errors():
    ...
    with GitRepoFixture():
        ...
        with patch.object(main, 'run_git_command', side_effect=mock_run_git_command):
            ...
            assert len(commit_commands) == 1, f"Expected 1 commit attempt, got {len(commit_commands)}"
            assert len(reset_commands) == 1, f"Expected 1 reset after syntax error, got {len(reset_commands)}"
...
def test_file_handler_ignore_patterns():
    ...
    with GitRepoFixture():
        ...
        for path, should_ignore in test_cases:
            ...
            assert result == should_ignore, f"Path {path}: expected ignore={should_ignore}, got {result}"
