...
class TestEnhancedTypes:
    ...
    def test_valid_commit_message(self):
        ...
        assert (
            process_commit_msg(long_first_line + "\nDetailed description")
            == long_first_line + "\nDetailed description"
        )
...
class TestResultTypes:
    ...
    def test_parsed_message(self):
        ...
        result = ParsedMessage(
            title=PRTitle("Fix bug"), body=PRBody("Fix bug\n\nDetailed description")
        )
...
class TestStateUtils:
    ...
    def test_get_state_name(self):
        ...
        assert (
            get_state_name(DirtyState(branch_name="main", changes="M file.py"))
            == "dirty"
        )
...
class TestEventTypes:
    ...
    def test_commit_event(self):
        ...
        empty_event = CommitEvent(
            timestamp=datetime.now(), files_changed=[], branch_name="vibe-123"
        )
    ...
    def test_state_transition_event(self):
        ...
        no_change = StateTransitionEvent(
            from_state=IdleState, to_state=IdleState, timestamp=datetime.now()
        )
