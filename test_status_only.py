...
from vibe_status_only import VibeSession, vibe_status, run_git_command, find_git_repo
...
def test_vibe_status_uses_found_repo_path():
    ...
    with GitRepoFixture():
        ...
        with unittest.mock.patch('vibe_status_only.find_git_repo') as mock_find, \
            ...
            mock_run.assert_called_with(["branch", "--show-current"], Path("/fake/repo"))
...
def test_find_git_repo_exact_path_check():
    ...
    with unittest.mock.patch('vibe_status_only.Path') as MockPath:
        ...
        mock_cwd.__ne__.side_effect = [True, False]  # First iteration: True, then False to exit loop
